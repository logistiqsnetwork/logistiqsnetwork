...
console.log(`LOGISTIQS NETWORK serving on http://${HOST}:${String(PORT)}`);

// Start the agent scheduler (runs daily at 6 AM UTC)
try {
  const { startScheduler } = await import("./src/agents/scheduler.js");
  startScheduler();
} catch (err) {
  console.warn("[Serve] Agent scheduler not started (may need build first):", err);
}
